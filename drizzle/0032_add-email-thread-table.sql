CREATE TYPE "public"."inngest_event_type_enum" AS ENUM('round-started', 'round-submitted', 'lottery-intervened', 'draft-concluded', 'draft-finalization', 'user-assigned');--> statement-breakpoint
...
ALTER TABLE "email"."email_thread" ADD CONSTRAINT "email_thread_recipient_user_id_user_id_fk" FOREIGN KEY ("recipient_user_id") REFERENCES "drap"."user"("id") ON DELETE cascade ON UPDATE cascade;