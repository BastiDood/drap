...
export class Database implements Loggable {
    ...
    @timed async initUser(email: string) {
        await this.#db.insert(schema.user).values({ email }).onConflictDoNothing({ target: schema.user.email });
    }

    @timed async upsertOpenIdUser(email: string, uid: string, given: string, family: string, avatar: string) {
        return await this.#db
            .insert(schema.user)
    ...
    @timed async updateLabQuotas(quotas: Iterable<readonly [string, number]>) {
        for (const [id, quota] of quotas) await this.#db.update(schema.lab).set({ quota }).where(eq(schema.lab.id, id));
    }
    ...
    @timed async getValidStaffEmails() {
        const results = await this.#db
            ...
            .from(schema.user)
            .where(and(eq(schema.user.isAdmin, true), isNull(schema.user.labId), isNotNull(schema.user.googleUserId)));
    ...
    @timed async getLabMembers(labId: string) {
        ...
        const heads = await this.#db.query.user.findMany({
            columns: { email: true, givenName: true, familyName: true, avatarUrl: true, studentNumber: true },
            where: and(isNotNull(schema.user.id), eq(schema.user.labId, labId), eq(schema.user.isAdmin, true)),
            orderBy: ({ familyName }) => familyName,
        });

        const members = await this.#db.query.user.findMany({
            columns: { email: true, givenName: true, familyName: true, avatarUrl: true, studentNumber: true },
            where: and(isNotNull(schema.user.id), eq(schema.user.labId, labId), eq(schema.user.isAdmin, false)),
            orderBy: ({ familyName }) => familyName,
        });
    ...
    @timed async getLabAndRemainingStudentsInDraftWithLabPreference(draftId: bigint, labId: string) {
        ...
        const researchers = await this.#db
            ...
            .where(and(eq(schema.facultyChoiceUser.draftId, draftId), eq(schema.facultyChoiceUser.labId, labId)));
    ...
    @timed async autoAcknowledgeLabsWithoutPreferences(draftId: bigint) {
        ...
        const draftedCte = this.#db.$with('_drafted').as(
            this.#db
                ...
                .from(draftsCte)
                .innerJoin(schema.facultyChoiceUser, eq(draftsCte.draftId, schema.facultyChoiceUser.draftId))
        ...
        const preferredCte = this.#db.$with('_preferred').as(
            this.#db
                .select({ labId: preferredSubquery.labId, preferrers: countDistinct(preferredSubquery.studentUserId) })
        ...
        await this.#db.insert(schema.facultyChoice).select(
            this.#db
                ...
                .where(
                    or(gte(sql`coalesce(${draftedCte.draftees}, 0)`, schema.lab.quota), eq(preferredCte.preferrers, 0)),
                ),
        );
    }
    ...
    @timed async getLabQuotaAndSelectedStudentCountInDraft(did: bigint, lid: string) {
        ...
        const draftCount = await this.#db
            ...
            .from(schema.facultyChoiceUser)
            .where(and(eq(schema.facultyChoiceUser.draftId, did), eq(schema.facultyChoiceUser.labId, lid)))
            .then(assertSingle);
    ...
    @timed async randomizeRemainingStudents(draftId: bigint) {
        const results = await this.#db
            ...
            .where(and(eq(schema.studentRank.draftId, draftId), isNull(schema.facultyChoiceUser.studentUserId)))
            .orderBy(sql`random()`);
    ...
    @timed async getCandidateSenders() {
        return await this.#db
            ...
            .where(and(isNotNull(schema.user.id), eq(schema.user.isAdmin, true), isNull(schema.user.labId)))
            .orderBy(schema.user.familyName);
    }
    ...
    @timed async getDesignatedSenderCredentials() {
        return await this.#db
            ...
            .where(and(isNotNull(schema.user.googleUserId), eq(schema.user.isAdmin, true), isNull(schema.user.labId)))
            .then(assertOptional);
    }

    @timed async upsertCandidateSender(userId: string, expiration: Date, accessToken: string, refreshToken?: string) {
        const query =
     ...
     */
    @timed async insertFacultyChoice(draftId: bigint, labId: string, facultyUserId: string, studentUserIds: string[]) {
        ...
        strictEqual(facultyChoiceRowCount, 1, 'insertFacultyChoice::facultyChoice => unexpected insertion count');

        if (studentUserIds.length === 0) return;

        const { rowCount: facultyChoiceUserRowCount } = await this.#db.insert(schema.facultyChoiceUser).values(
            studentUserIds.map(
                studentUserId =>
                    ({
                        draftId,
    ...
    @timed async insertLotteryChoices(
        ...
        await this.#db
            ...
            .onConflictDoUpdate({
                target: [schema.facultyChoice.draftId, schema.facultyChoice.round, schema.facultyChoice.labId],
