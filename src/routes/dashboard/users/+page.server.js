...
export async function load({ locals: { db }, parent }) {
    ...
    const [labs, faculty] = await Promise.all([db.getLabRegistry(), db.getRegisteredFaculty()]);
    return { labs, faculty };
}

export const actions = {
    async invite({ locals: { db }, request }) {
        ...
        if (await db.inviteNewUser(email, lab)) return;
        return fail(409);
    },
};
