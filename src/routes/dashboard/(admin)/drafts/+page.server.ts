...
export async function load({ locals: { session }, parent }) {
  ...
  logger.info('labs fetched', { 'lab.count': labs.length });
  ...
  logger.info('draft records fetched', {
...
export const actions = {
  async init({ locals: { session }, request }) {
    ...
    logger.info('initializing draft', {
  ...
  async start({ locals: { session }, request }) {
    ...
    if (!isValid) {
      logger.error('invalid total lab quota');
      return fail(498);
    }

    const data = await request.formData();
    const draftId = BigInt(validateString(data.get('draft')));
    logger.info('starting draft', { 'draft.id': draftId.toString() });
    ...
    if (studentCount <= 0) {
      logger.error('no students in draft');
      return fail(497);
    }
  ...
  async intervene({ locals: { session }, request }) {
    ...
    const data = await request.formData();
    const draftId = BigInt(validateString(data.get('draft')));
    logger.info('intervening draft', { 'draft.id': draftId.toString() });
    data.delete('draft');

    const pairs = Array.from(mapRowTuples(data));
    if (pairs.length === 0) {
      logger.info('no pairs to intervene');
      return;
    }

    logger.info('intervening draft with pairs', { 'draft.pair_count': pairs.length });
  ...
  async conclude({ locals: { session }, request }) {
    ...
    const data = await request.formData();
    const draftId = BigInt(validateString(data.get('draft')));
    logger.info('concluding draft', { 'draft.id': draftId.toString() });
    ...
    try {
      const notifications = await begin(db, async db => {
        ...
        logger.info('round-robin schedule generated', { 'draft.schedule.length': schedule.length });

        const studentUserIds = await randomizeRemainingStudents(db, draftId);
        logger.info('randomized student queue generated', {
        ...
        if (pairs.length > 0) {
          logger.info('inserting lottery choices', { 'draft.pair_count': pairs.length });
          ...
          logger.warn('no students remaining in the lottery');
        }
        ...
        logger.info('draft results synced', { 'draft.result_count': results.length });
        notifications.push(
