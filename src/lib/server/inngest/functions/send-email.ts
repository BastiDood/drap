        ...
        await tracer.asyncSpan('refresh-sender-credentials', async () => {
          ...
          switch (event.name) {
            ...
            case 'draft/user.assigned':
              ...
              break;
            default: {
              const error = new NonRetriableError(`unknown event type: ${event.name}`);
              logger.error('unknown event type', error, { 'inngest.event.name': event.name });
              throw error;
            }
          }
          ...
          const { id, threadId, labelIds } = await client.sendEmail(mime);
          logger.info('email sent successfully', {
            'email.message.id': id,
            'email.message.thread_id': threadId,
            'email.message.label_ids': labelIds,
          });
        }),
    ),
);
